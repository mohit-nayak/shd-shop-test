...
import { storeFilteredProducts } from "../redux/action/product";
...
const Shop = ({
	...
	let [pages, setPages] = useState(
		Math.ceil(filteredProducts.items.length / limit)
	);
	let [currentPage, setCurrentPage] = useState(1);

	const cratePagination = () => {
		// set pagination
		let arr = new Array(Math.ceil(filteredProducts.items.length / limit))
			.fill()
			.map((_, idx) => idx + 1);

		setPagination(arr);
		setPages(Math.ceil(filteredProducts.items.length / limit));
	};

	const startIndex = currentPage * limit - limit;
	const endIndex = startIndex + limit;
	const getPaginatedProducts = filteredProducts.items.slice(
		startIndex,
		endIndex
	);
	...
	const selectChange = (e) => {
		...
		setCurrentPage(1);
		setPages(Math.ceil(filteredProducts.items.length / Number(e.target.value)));
	};

	useEffect(() => {
		storeFilteredProducts(products, searchTerm, productFilters);
		cratePagination();
	...
	return (
		<>
			<Layout noBreadcrumb="d-none">
				<Head>
					<title>Shop | Bio Hundeshop</title>
